...
Many things are changing currently since we are finding issues in hosting projects, especially with memory management in cache but core classes are already stable for a while. At the core of library are basic interfaces that provide access to Google services using documented HTTP/JSON protocol. Those classes are geneated from doc-specifications called STONE. STONE was designed by Dropbox as declarative languague to maintain their APIs. We addapted STONE python machinery for C++11/Qt and Dropbox interfaces first and then for selected Google APIs. Our main goal was Gmail and GDrive as future engines for Ardi-application. At one point we desided to abstruct caching classes from Ardi into googleQt as well and that is where we hit complexity issue since all caching code is hand-made and no python script would be able to automate development at least at this point of our understanding of the project. We estimated development of stable GMail cache core interfaces for GDrive, GTask and maybe some more APIs with autotests and sample projects in 1 year. 7 months already passed and we have beta of hosting application using all this code (except GTasks). It just happened to crash sometimes probably because of raw pointers curse but we should be able to handle it as well.
