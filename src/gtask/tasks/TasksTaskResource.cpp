...
std::unique_ptr<TaskResource> TaskResource::EXAMPLE(int context_index, int parent_context_index){
    ...
    rv->m_parent = QString("parent_%1").arg(example_idx);
    rv->m_position = QString("position_%1").arg(example_idx);
    rv->m_notes = QString("notes_%1").arg(example_idx);
    rv->m_status = QString("status_%1").arg(example_idx);
